...
from BEL42_TS.checkAI import _summaryAI_modelcall
...
class Article:
    def __init__(self, title: str, href: str, body: str, sim_percentage: float = 0.00):
        self.title = title
        self.href = href
        self.body = body
...
def fsimiliarity(pquery1: str = "", pquery2: str = ""):
    ...
    simperc = cosine_similarity(ptensor1.mean(dim=1), ptensor2.mean(dim=1)) * 100
    return simperc.mean().item()
...
class SearcherWithInherence:
    ...
    def activate(self) -> List[Article] | str:
        ...
        outputs = sorted(cresults, key=lambda cres: cres.sim_percentage, reverse=True)
        outputs = outputs[:5]
